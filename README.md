...
* Open RStudio and enter the following commands to install **bstr version 0.5**.
* You can pull directly from the BrainSuite server (Online Installation), or you can download the bstr_0.5.tar.gz file directly and install from your downloads folder.
* Do not untar on uncompress bstr_0.5.tar.gz -- the installer needs it in this format.
...
```
install.packages('devtools')
devtools::install_local('/path/to/bstr_0.5.tar.gz')
```
...
```
install.packages('devtools')
devtools::install_local('C:\\path\\to\\bstr_0.5.tar.gz')
```
or
```
install.packages('devtools')
devtools::install_local('C:/path/to/bstr_0.5.tar.gz')
```
